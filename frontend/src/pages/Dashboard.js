...
// Hook customizado final para busca de dados
...
function Dashboard() {
  ...
  // Estilo padrão para os tooltips de tema escuro
  const tooltipStyle = {
    ...
    <div className="dashboard">
      ...
      <div className="charts-grid">
        <ChartWrapper title="Operações por Status" endpoint="operacoes_por_status">
            {(data) => (<ResponsiveContainer width="100%" height={400}><BarChart data={data.map(item => ({ ...item, name: abbreviateStatus(item.name) }))}><XAxis dataKey="name" stroke="var(--text-secondary)" interval={0} tick={{ fontSize: 12 }} /><YAxis stroke="var(--text-secondary)" tickFormatter={formatYAxisTick} />
            <Tooltip cursor={{ fill: 'rgba(255, 255, 255, 0.05)' }} contentStyle={tooltipStyle} itemStyle={{ color: 'white' }} />
            <Bar dataKey="value" radius={[4, 4, 0, 0]}>{data.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</Bar></BarChart></ResponsiveContainer>)}
        </ChartWrapper>
        <ChartWrapper title="Top 10 Estados por Valor de Frete" endpoint="valor_frete_por_uf">
            {(data) => (<ResponsiveContainer width="100%" height={400}><BarChart data={data}><XAxis dataKey="name" stroke="var(--text-secondary)" interval={0} tick={{ fontSize: 12 }} /><YAxis stroke="var(--text-secondary)" tickFormatter={(value) => `R$${formatYAxisTick(value)}`} />
            <Tooltip formatter={(value) => formatCurrency(value)} cursor={{ fill: 'rgba(255, 255, 255, 0.05)' }} contentStyle={tooltipStyle} />
            <Bar dataKey="value" radius={[4, 4, 0, 0]} fill="#2dce89" /></BarChart></ResponsiveContainer>)}
        </ChartWrapper>
        <ChartWrapper title="Operações Criadas (Últimos 30 dias)" endpoint="operacoes_por_dia">
            {(data) => (<ResponsiveContainer width="100%" height={400}><LineChart data={data}><CartesianGrid strokeDasharray="3 3" stroke={"var(--border-color)"} /><XAxis dataKey="name" stroke="var(--text-secondary)" /><YAxis stroke="var(--text-secondary)" />
            <Tooltip contentStyle={tooltipStyle} />
            <Legend /><Line type="monotone" dataKey="value" name="Nº de Operações" stroke="#ff8d4e" /></LineChart></ResponsiveContainer>)}
        </ChartWrapper>
        <ChartWrapper title="Top 5 Clientes por Valor de Mercadoria" endpoint="top_clientes_por_valor">
            {(data) => (<ResponsiveContainer width="100%" height={400}><PieChart><Pie data={data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={150} label={({ name, percent }) => `${name.split(' ')[0]} (${(percent * 100).toFixed(0)}%)`}>{data.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</Pie>
